...
export default function EditProductScreen({ route, navigation }) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await axios.put(`http://192.168.18.56:5000/api/producto/${product.codigo}`, {
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            setLoading(true);
            try {
              await axios.delete(`http://192.168.18.56:5000/api/producto/${product.codigo}?id_agricultor=${user.id_agricultor}`);
